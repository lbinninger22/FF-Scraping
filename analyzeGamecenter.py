            ...
            with open(filepath, 'r', newline='') as file:
                ...
                for row in reader:
                    ...
                    if margin > largest_margin:
                        print("Week " + filename[:-4] + " " + row['Owner'] + " has a new blowout with a " + f"{margin:.2f}" + " point win over " + row['Opponent'])
                        largest_margin = margin
                    if margin > 0 and margin < narrowest_margin:
                        print("Week " + filename[:-4] + " " + row['Owner'] + " has a new narrow victory with a " + f"{margin:.2f}" + " point win over " + row['Opponent'])
